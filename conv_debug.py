...
network = Convnetwork(input_size=(list(x_train[0].shape)), output_size=10, dense_layer=layer1, conv_layer=conv_layer1, weightinit=He, activation=LeakyRelu, batchnorm=True, toba=True, drop_rate=[0.26,0.35], regularize=["l2",0.0005])
test = Trainer(network, optimizer=opt2, data=data, check=5, scheduler=exp)
test.fit(10)
#prev = copy.deepcopy(test)
test.coco_sort([0,100,50,25],["Affine2","Affine3","Affine4"])
