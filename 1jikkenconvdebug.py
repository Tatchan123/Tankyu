...
toba_option = {"rmw_type":"coco_ls","epsilon":[0.9,0.9,0.9,0.9,0.9],"complement":True,"rmw_layer":["C2","C3",1,2,3],"delete_n":[3,3,6,10,10,10]}

network = Convnetwork(input_size=(list(x_train[0].shape)), output_size=10, dense_layer=layer1, conv_layer=conv_layer1, weightinit=He, activation=Relu, batchnorm=True, drop_rate=[0.26,0.33], regularize=["l2",0.0005])
test = Trainer(network, step=[20,"toba"], optimizer=opt2, data=data, check=2, tobaoption=toba_option, scheduler=exp)
test.fit()
