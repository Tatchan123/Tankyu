...
network = Convnetwork(input_size=(list(x_train[0].shape)), output_size=10, dense_layer=layer1, conv_layer=conv_layer1, weightinit=He, activation=Relu, batchnorm=True, drop_rate=[0.25,0.33], regularize=["l2",0.0005])
test = Trainer(network, step=[50], optimizer=opt2, data=data, check=2, tobaoption=toba_option, scheduler=exp)
test.fit()
